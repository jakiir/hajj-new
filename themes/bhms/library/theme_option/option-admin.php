		...
		class JestroCore {
			...
			function printAdminScripts () {
				if ( $_GET['page'] == basename(__FILE__) ) {
					wp_enqueue_style( 'jestro', get_bloginfo( 'template_directory' ).'/library/theme_option/stylesheets/admin.css' );
					wp_enqueue_script( 'jestro', get_bloginfo( 'template_directory' ).'/library/theme_option/javascripts/admin.js', array( 'jquery' ) );
			...
			function adminPage () {
				// global $themename, $shortname, $options;
				if ( $_REQUEST['saved'] ) echo '<div id="message" class="updated fade"><p><strong>' . $this->themename . __( ' settings saved!', 'hajj' ) . '</strong></p></div>';
				if ( $_REQUEST['reset'] ) echo '<div id="message" class="updated fade"><p><strong>' . $this->themename . __( ' settings reset.', 'hajj' ) . '</strong></p></div>'; ?>

				<div id="v-options">
					<div class="v-top clear">
						<h1 class="v-trial"><?php echo $this->themename; ?> <?php _e( 'Options', 'hajj' ); ?></h1>
					</div>
...
<?php
		...
		<div class="v-save-button submit">
			...
			<input class="button-primary" type="submit" value="<?php _e( 'Save changes', 'hajj' ); ?>" name="save"/>
...
<div class="v-option">
	...
	<div class="v-option-body clear">
		...
		<div class="v-field colorpicker clear <?php if ($pro == 'true' ) echo 'pro' ?>">
			...
			<div class="v-field-d"><span><?php echo $this->options[$i]["desc"]; ?></span></div>
			<label for="<?php echo $this->options[$i]["id"]; ?>"><?php echo $this->options[$i]["name"]; ?><?php if ($pro != 'true' ) { ?> <a href="javascript:return false;" onclick="toggleColorpicker (this, '<?php echo $this->options[$i]["id"]; ?>', 'open', '<?php _e( 'show color picker', 'hajj' ); ?>', '<?php _e( 'hide color picker', 'hajj' ); ?>' )"><?php _e( 'show color picker', 'hajj' ); ?><?php } ?></a></label>
		...
		<div class="v-field textarea clear <?php if ($pro == 'true' ) echo 'pro' ?>">
			...
			?>
					<div class="v-save-button submit">
						<input type="submit" value="<?php _e( 'Save changes', 'hajj' ); ?>" name="save"/>
					</div><!--end v-save-button-->
				</div>
			</div>
			<div class="v-saveall-button submit">
				<input class="button-primary" type="submit" value="<?php _e( 'Save all changes', 'hajj' ); ?>" name="save"/>
			</div>
			</form>
			<div class="v-reset-button submit">
				<form method="post">
					<input type="hidden" name="action" value="reset" />
					<input class="v-reset" type="submit" value="<?php _e( 'Reset all options', 'hajj' ); ?>" name="reset"/>
				</form>
			</div>
