 ...
 *
 * Be sure to replace all instances of 'hajjsite_' with your project's prefix.
 ...
 */
function hajjsite_show_if_front_page( $cmb ) {
 ...
 * @return bool              True if metabox should show
 */
function hajjsite_hide_if_no_cats( $field ) {
 ...
 */
function hajjsite_render_row_cb( $field_args, $field ) {
 ...
 * @param  CMB2_Field $field      The field object.
 */
function hajjsite_display_text_small_column( $field_args, $field ) {
	?>
 ...
 * @param  CMB2_Field $field      Field object.
 */
function hajjsite_before_row_if_2( $field_args, $field ) {
...
add_action( 'cmb2_admin_init', 'hajjsite_register_demo_metabox' );
/**
 * Hook in and add a demo metabox. Can only happen on the 'cmb2_admin_init' or 'cmb2_init' hook.
 */
function hajjsite_register_demo_metabox() {
	$prefix = 'hajj_';
	...
	/*$cmb_demo = new_cmb2_box( array(
		...
		// 'show_on_cb' => 'hajjsite_show_if_front_page', // function should return a bool value
		// 'context'    => 'normal',
		// 'priority'   => 'high',
		...
		// 'classes_cb' => 'hajjsite_add_some_classes', // Add classes through a callback.
	) );
	...
	$cmb_demo->add_field( array(
		...
		'id'         => $prefix . 'text',
		'type'       => 'text',
		'show_on_cb' => 'hajjsite_hide_if_no_cats', // function should return a bool value
	...
	$cmb_demo->add_field( array(
		...
		// );
		// 'display_cb' => 'hajjsite_display_text_small_column', // Output the display of the column values through a callback.
	) );
	...
	$cmb_demo->add_field( array(
		...
		'type' => 'text',
		'render_row_cb' => 'hajjsite_render_row_cb',
	) );
	...
	$cmb_demo->add_field( array(
		...
		'type'         => 'text',
		'before_row'   => 'hajjsite_before_row_if_2', // callback.
...
add_action( 'cmb2_admin_init', 'hajjsite_register_hajj_info_page_metabox' );
/**
...
function hajjsite_register_hajj_info_page_metabox() {
	 ...
	 */
	$cmb_hajj_info_page = new_cmb2_box( array(
		'id'           => $prefix . 'metabox',
		'title'        => esc_html__( 'Hajj Info Metabox', 'cmb2' ),
		'object_types' => array( 'page' ), // Post type
		'context'      => 'normal',
		'priority'     => 'high',
		'show_names'   => true, // Show field names on the left
	...
	$cmb_hajj_info_page->add_field( array(
		'name' => esc_html__( 'Hajj Pilgrim Quota Title', 'cmb2' ),
		'desc' => esc_html__( 'Write Hajj Pilgrim Quota Title', 'cmb2' ),
		'id'   => $prefix . 'quota',
		'type' => 'text',
	) );
	$cmb_hajj_info_page->add_field( array(
		'name' => esc_html__( 'Hajj Pilgrim Quota Total', 'cmb2' ),
		'desc' => esc_html__( 'Write Hajj Pilgrim Quota Total', 'cmb2' ),
		'id'   => $prefix . 'quota_total',
		'type' => 'text',
	) );
	
	$cmb_hajj_info_page->add_field( array(
		'name' => esc_html__( 'Govt. Registered Title', 'cmb2' ),
		'desc' => esc_html__( 'Write Govt. Registered Title', 'cmb2' ),
		'id'   => $prefix . 'govt_registered_title',
		'type' => 'text',
	) );
	$cmb_hajj_info_page->add_field( array(
		'name' => esc_html__( 'Govt. Registered Total', 'cmb2' ),
		'desc' => esc_html__( 'Govt. Registered Total', 'cmb2' ),
		'id'   => $prefix . 'govt_registered_title_total',
		'type' => 'text',
	) );
	
	$cmb_hajj_info_page->add_field( array(
		'name' => esc_html__( 'Pvt. Registered Title', 'cmb2' ),
		'desc' => esc_html__( 'Write Pvt. Registered Title', 'cmb2' ),
		'id'   => $prefix . 'pvt_registered_title',
		'type' => 'text',
	) );
	$cmb_hajj_info_page->add_field( array(
		'name' => esc_html__( 'Pvt. Registered Total', 'cmb2' ),
		'desc' => esc_html__( 'Write Pvt. Registered Total', 'cmb2' ),
		'id'   => $prefix . 'pvt_registered_total',
		'type' => 'text',
	) );

}

add_action( 'cmb2_admin_init', 'hajjsite_register_repeatable_group_field_metabox' );
/**
 ...
 */
function hajjsite_register_user_profile_metabox() {
	$prefix = 'hajjsite_user_';
...
add_action( 'cmb2_admin_init', 'hajjsite_register_taxonomy_metabox' );
/**
 * Hook in and add a metabox to add fields to taxonomy terms
 */
function hajjsite_register_taxonomy_metabox() {
	$prefix = 'hajjsite_term_';
...
add_action( 'cmb2_admin_init', 'hajjsite_register_theme_options_metabox' );
/**
 * Hook in and register a metabox to handle a theme options page and adds a menu item.
 */
function hajjsite_register_theme_options_metabox() {
	$prefix = 'hajj_';
	/**
	...
	$cmb_options = new_cmb2_box( array(
		...
		// 'message_cb'      => 'hajjsite_options_page_message_callback',
	) );
	...
	$cmb_options->add_field( array(
		'name'    => esc_html__( 'Site Logo', 'cmb2' ),
		'desc'    => esc_html__( 'Site Logo (optional)', 'cmb2' ),
		'id'      => 'site_logo',
		'type'    => 'file',
		'default' => get_template_directory_uri().'/images/batworld-logo.png',
	) );
	
	$cmb_options->add_field( array(
        'name'     => __( 'Launcher popup enable', 'cmb2' ),
        'desc'     => __( 'Please check this if you enable luncher popup (optional)', 'cmb2' ),
        'id'       => $prefix . 'luncher_popup',
        'type'     => 'checkbox'
    ) );
	$cmb_options->add_field( array(
		'name'    => esc_html__( 'Popup image', 'cmb2' ),
		'desc'    => esc_html__( 'Upload luncher popup image (optional)', 'cmb2' ),
		'id'      => $prefix . 'luncher_popup_image',
		'type'    => 'file'
	) );
	$cmb_options->add_field( array(
		'name' => esc_html__( 'Popup text', 'cmb2' ),
		'desc' => esc_html__( 'Write luncher popup message (optional)', 'cmb2' ),
		'id'   => $prefix . 'luncher_popup_text',
		'type' => 'textarea',
	) );
...
function hajjsite_get_option( $key = '', $default = false ) {
	if ( function_exists( 'hajj_theme_options' ) ) {
		// Use cmb2_get_option as it passes through some key filters.
		return cmb2_get_option( 'hajj_theme_options', $key, $default );
	}
	// Fallback to get_option if CMB2 is not loaded yet.
	$opts = get_option( 'hajj_theme_options', $default );
	$val = $default;
	if ( 'all' == $key ) {
		$val = $opts;
 ...
 * }
 */
function hajjsite_options_page_message_callback( $cmb, $args ) {
 ...
 */
function hajjsite_limit_rest_view_to_logged_in_users( $is_allowed, $cmb_controller ) {
...
add_action( 'cmb2_init', 'hajjsite_register_rest_api_box' );
/**
 ...
 */
function hajjsite_register_rest_api_box() {
	$prefix = 'hajjsite_rest_';
	...
	/*$cmb_rest = new_cmb2_box( array(
		...
		// 'get_box_permissions_check_cb' => 'hajjsite_limit_rest_view_to_logged_in_users',
	) );
